...
void dijkstra(int s,vpl adj[], vl & dis){
    ...
    while(!qu.empty()){
        ...
        for(auto edge : adj[top.second]){
            if(dis[edge.first]> dis[top.second]+edge.sec){
                dis[edge.first]=dis[top.second]+edge.sec;
                qu.push({-dis[edge.first],edge.first});
            }
        }
    }
}
