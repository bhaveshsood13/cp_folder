...
const ll INF = 1e18;
 ...
 // let res be the number which cannot be formed from all subsets till pos i-1; then for i 
 // 1) We decide that ‘res’ is the final result:
//   If arr[i] is greater than ‘res’, then we found the gap which is 
// ‘res’ because the elements after arr[i] are also going to be greater than ‘res’.
